...
from G_code import _03_GLM as glm
...
def main() -> None:
    ...
    _log("Running GLM analysis stage")
    glm_func_path = expected if expected.exists() else paths["fmri"]

    glm.run_glm_analysis(
        project_root=project_root,
        func_path=glm_func_path,
        t1w_path=anat_path,
        events_lr=project_root / "fMRI" / "tfMRI_MOTOR_LR" / "events_LR.csv",
        events_rl=project_root / "fMRI" / "tfMRI_MOTOR_RL" / "events_RL.csv",
    )

    _log("Pipeline complete (ICA + GLM).")
